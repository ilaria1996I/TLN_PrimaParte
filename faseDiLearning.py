...
def riempi_dict_quando_non_è_una_riga_vuota(tag_precedente,numero_di_riga,parola,tag,tag_della_parola,tag_che_precede_tag):
    if numero_di_riga == 0: #se è la prima parola della frase!
        tag_precedente = tag
        if tag in tag_che_precede_tag['FIRST']:
            tag_che_precede_tag['FIRST'][tag] += 1
        else:
            tag_che_precede_tag['FIRST'][tag] = 1

            
    #C(ti, wi) -----> mi permetterà di fare il conteggio quando la parola W viene dato tag T 
    if parola not in tag_della_parola:
        tag_della_parola[parola] = dict()
    if tag in tag_della_parola[parola]:
        tag_della_parola[parola][tag] += 1
    else:
        tag_della_parola[parola][tag] = 1
    ...
    if numero_di_riga > 0:
        if tag_precedente not in tag_che_precede_tag:
            tag_che_precede_tag[tag_precedente] = dict() 
        if tag in tag_che_precede_tag[tag_precedente]:
            tag_che_precede_tag[tag_precedente][tag] += 1
        else:
            tag_che_precede_tag[tag_precedente][tag] = 1
    tag_precedente = tag
    ...
    with open(path, 'r', encoding='utf-8') as testo:
        for i, line in enumerate(testo):    
            ...
            if(line == '\n'): # se è una riga vuota è finita la frase
                if "LAST" not in tag_che_precede_tag[tag]:
                    tag_che_precede_tag[tag]['LAST'] = 1
                else:
                    tag_che_precede_tag[tag]['LAST'] += 1
...
def fase_di_learning(path):
    ...
    #da questo punto in poi si calcola le probabilità usando i contatori precedenti 
    
    #serve per calcolare P(ti | ti-1) usando "tag_che_precede_tag"
    ...
    #serve per calcolare P(wi | ti) usando "tag_della_parola"
    prob_parola_con_tag = dict()
    for parola in tag_della_parola:
        for tag_corrente in tag_della_parola[parola]:
            if parola not in prob_parola_con_tag:
                prob_parola_con_tag[parola] = dict()  
