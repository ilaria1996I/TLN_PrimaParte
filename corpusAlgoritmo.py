...
def algoritmoDiViterbi(frase:list, trans_prob:dict, emission_prob:dict, parole_sconosciute:dict ):
    ...
    for i in range(numParoleDellaFrase):
        if(i>0):
            ...
            for state in trans_prob.keys():
                if(state != 'FIRST'):
                    mVit, mPath, tag = scegliValoreDiViterbiMiglioreFinoAlloStatoCorrente(vit, frase[i], i-1, tag,state, trans_prob, emission_prob, parole_sconosciute)
    ...
    for tagPreced in trans_prob.keys():
        if tagPreced != 'FIRST':
            if( 'LAST' in trans_prob[tagPreced]): #se siamo qui è l'ultima parola
                ...
                if ViterbiCambiato > mPath:
                    mPath = ViterbiCambiato
                    lastTag = tagPreced 
    vit['LAST'] = mPath
...
def scegliValoreDiViterbiMiglioreFinoAlloStatoCorrente(vit, parola, i, tag, state,trans_prob, emission_prob, parole_sconosciute):
    mVit = -100000
    mPath = -100000
    valoreViterbiCambiato = 0
    pathViterbiCambiato = 0
        ...
        if tagPreced != 'FIRST':
            if parola in parole_sconosciute:
                if state in parole_sconosciute[parola] and state in trans_prob[tagPreced]:
                    ...
                    if valoreViterbiCambiato > mVit:mVit = valoreViterbiCambiato
                    
                    pathViterbiCambiato = vit[tagPreced][i]+ trans_prob[tagPreced][state]
                    if pathViterbiCambiato > mPath:
                        mPath = pathViterbiCambiato
                        tag = tagPreced
            else:
    ...
    with open(pathTest, 'r', encoding='utf-8') as test:
            for line in test:
                ...
                if line == '\n':
                    ...
                    sm1 = algoritmoDiViterbi(frase, prob_che_un_tag_precedi_un_altro_tag, prob_parola_con_tag, smoothing1)[0]
                    sm2 = algoritmoDiViterbi(frase, prob_che_un_tag_precedi_un_altro_tag, prob_parola_con_tag, smoothing2)[0]
                    sm3 = algoritmoDiViterbi(frase, prob_che_un_tag_precedi_un_altro_tag, prob_parola_con_tag, smoothing3)[0]
                    sm4 = algoritmoDiViterbi(frase, prob_che_un_tag_precedi_un_altro_tag, prob_parola_con_tag, smoothing4)[0]
